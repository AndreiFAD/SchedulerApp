 ...
 */
@Stateless
public class BatchJobE implements BatchJobInterface
{
    static final Logger logger = Logger.getLogger("BatchJobE");
    ...
    /**
     *
     * @param jobInfo readformjson job parameters
     */
    @Override
    public void jobrunn(JobInfo jobInfo){ 
                ...
                logger.log(Level.INFO, "Start of BatchJobE "+jobid+" "+jobname+" at {0}...", new Date());
                ...
                try {
                    ...
                    if (needjobcheck.equals("true") && (jobid_last_time.equals("still has not run") || jobid_last_time.equals("never ran")) && needjobid_last_time.equals("completed today")){  // ha kell figyelni a függőséget és ő még ma nem futott
                        ...
                        try {
                                vid = l.logentry(Integer.parseInt(jobid), jobname, "Start of BatchJobE "+jobid+" "+jobname+" at " + new Date() + "...");
                            } catch (SQLException ex) {
                                Logger.getLogger(BatchJobE.class.getName()).log(Level.SEVERE, null, ex);
                            }
                        ...
                        try {
                            ...
                            System.err.println("Az 'E' batch lefutott "+jobInfo.getJobName()+" - "+jobInfo.getDescription());
                            logger.log(Level.INFO, "Running job: {0}", jobInfo);
                            Thread.sleep(60000);
                            
                            try {
                                    l.logclose(vid, "Az 'E' batch lefutott "+jobInfo.getJobName(), "");
                                } catch (SQLException ex1) {
                                    Logger.getLogger(BatchJobE.class.getName()).log(Level.SEVERE, null, ex1);
                                }
                            
                            } else {
                                
                                 try {
                                    l.logclose(vid, "Az 'E' batch "+jobInfo.getJobName(), "hibával futott le: owner: "+ jobInfo.getUsername()+ ", error: "+results);
                                } catch (SQLException ex1) {
                                    Logger.getLogger(BatchJobE.class.getName()).log(Level.SEVERE, null, ex1);
                                }
                            Logger.getLogger(BatchJobE.class.getName()).log(Level.SEVERE, null, results);
                            ...
                            try {
                                    l.logclose(vid, "Az 'E' batch "+jobInfo.getJobName(), "hibával futott le: owner: "+ jobInfo.getUsername()+ ", error: "+ex.getMessage());
                                } catch (SQLException ex1) {
                                    Logger.getLogger(BatchJobE.class.getName()).log(Level.SEVERE, null, ex1);
                                }
                            Logger.getLogger(BatchJobE.class.getName()).log(Level.SEVERE, null, ex);
                ...
                } catch (Exception ex) {Logger.getLogger(BatchJobE.class.getName()).log(Level.SEVERE, null, ex);}
